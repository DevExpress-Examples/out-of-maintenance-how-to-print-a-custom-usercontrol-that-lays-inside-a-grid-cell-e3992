...
Namespace CustomControlInGrid
    ...
    Friend Class CustomRepositoryItem
        ...
        Private Sub UpdateControlRepositoreies()
            ...
            Dim editorList As List(Of BaseEdit) = FindEditors(_drawControl)
        ...
        Public Shared Sub Register()
            Call EditorRegistrationInfo.Default.Editors.Add(New EditorClassInfo(EditorName, GetType(CustomControl), GetType(CustomRepositoryItem), GetType(CustomControlViewInfo), New CustomControlPainter(), True))
        End Sub
        ...
        Public Overrides Function GetBrick(ByVal info As PrintCellHelperInfo) As VisualBrick
            Dim brick As VisualBrick = MyBase.GetBrick(info)
            info.Graph.DrawBrick(brick, info.Rectangle)
            Return TryCast(TryCast(DrawControl, IExport).GetBrick(info), VisualBrick)
        End Function
    End Class
End Namespace
